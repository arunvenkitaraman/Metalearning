...
fig_id='17Mar_5000_min3_min3_reg0'
#fig_id.to(device)
def main(args):
   ...
   # db = OmniglotNShot('omniglot', batchsz=32, n_way=5, k_shot=1, k_query=1, imgsz=w)
    ...
    metasgd.load_state_dict(checkpoint['metasgd_state_dict'])
    #krmaml.load_state_dict(checkpoint['krmaml_state_dict'])
    #krmaml_cos.load_state_dict(checkpoint['krmaml_cos_state_dict'])
    
    # load
    pretrained_dict =checkpoint['maml_state_dict']
    
    
    model_dict = krmaml.state_dict()
# 1. filter out unnecessary keys
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
# 2. overwrite entries in the existing state dict
    model_dict.update(pretrained_dict) 
# 3. load the new state dict
    krmaml.load_state_dict(model_dict)

    model_dict = krmaml_cos.state_dict()
# 1. filter out unnecessary keys
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
# 2. overwrite entries in the existing state dict
    model_dict.update(pretrained_dict) 
# 3. load the new state dict
    krmaml_cos.load_state_dict(model_dict)

    print('maml',maml.net.parameters()[0])
    print('krmaml',krmaml.net.parameters()[0])
    print('krmaml_cos',krmaml_cos.net.parameters()[0])
    
    #krmaml.load_state_dict(checkpoint['maml_state_dict'])
    #krmaml_cos.load_state_dict(checkpoint['maml_state_dict'])
    #metasgd.load_state_dict(checkpoint['metasgd_state_dict'])
    ...
    for step in range(args.epoch):
        ...
        if step % 50 == 0:
            ...
            plt.legend()
            fig_name='train_nmse_'+fig_id+'.jpg'
        ...
        if step % 500 == 0:
            ...
            plt.legend()
            fig_name='test_nmse_'+fig_id+'.jpg'
            plt.savefig(fig_name)  
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--reg', type=int, help='regula', default=0)
    argparser.add_argument('--reg2', type=int, help='regula', default=1e-6)
    argparser.add_argument('--reg_cos', type=int, help='regula', default=0)
    argparser.add_argument('--n_rff', type=int, help='n_rff', default=10000)
